...
Who participates in meetups? That's a pretty big question and can be approached in different ways. Start by breaking the question down into its parts:
Who -- students? developers? employed people? people seeking employment or career change? are they financially stable?
Participates -- what does it mean to participate? How should participation be quantified? RSVP's? Engagement on meetup site? How many people actually show up?
Meetups -- what topics should we focus on? some meetups are virtual and in person, how do you control for variation? what do meetups tell about the city?

Basically, it's a loaded question. I'm trying to focus on what do meetup levels tell about a city, specifically about the city's economic health.

## Hypothesis

Does a city's change in GDP (real GDP in chained dollars) correlate with the amount of meetups in a city?
Are more economically healthy cities hosting more meetups?

## Methodology

The Bureau of Economic Analysis provides a dashboard that allows users to filter and gather data that they need. 
For the meetup data, Augustina was able to provide the necessary information but Meetup also has an API as well as an interactive dashboard to make gathering data easier.
https://www.bea.gov/itable/
Regional Data > GDP & Personal Income > GDP by Metropolitan area > Real GDP in chained dollars > All Industries > US (and cities) > Percent change from preceding period > All Years
...
```{r load_data}
meetups <- read.csv("meetup_groups_by_city.csv")
...
```{r}
ggplot(my_data, aes(x=column_name_for_thing_to_count) +  geom_bar() +  xlab("Thing I'm Counting"))
```

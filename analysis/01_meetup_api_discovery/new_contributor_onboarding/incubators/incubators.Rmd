...
seeddb_companies <- read.csv("seeddb_companies.csv")
seeddb_companies_lnk <- seeddb_companies %>% filter(! is.na(angellist))

for(n in 1){#:nrow(seeddb_companies)){
  row <- seeddb_companies_lnk[n,]
  angellist_link <- as.character(row["angellist"])
  company <- as.character(row["Company"])
  ...
  print(paste("getting HTML for", acc))
  seeddb_company_html <- download.file(angellist_link, company_filename)

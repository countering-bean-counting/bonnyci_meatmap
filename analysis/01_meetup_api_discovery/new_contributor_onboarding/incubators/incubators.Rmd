...
```{r seeddb_accelerators_table, eval=FALSE}
...
```{r seeddb_accelerators_links, eval=FALSE}
...
```{r seeddb_accelerators_fix_cities, eval=FALSE}
...
```{r seeddb_accelerators_companies, eval=FALSE}
...
```{r get_company_location, eval=FALSE}
...
```{r, eval=FALSE}
incubators <- read.csv("seeddb_accelerators_usa.csv", stringsAsFactors = FALSE, na.strings = "")
metro_areas_all <- read.csv("../bls/qcew/qcew_msa_all.csv", stringsAsFactors = FALSE, na.strings = "")
...
incubators <- incubators %>%
  mutate(city=ifelse(city=="Greenville and Spartanburg", "Greenville", as.character(city)),
         city=ifelse(city=="Silicon Valley", "San Jose", as.character(city)),
         city=ifelse(city=="New York City"|city=="Brooklyn", "New York", as.character(city)))

# join on metro areas
incubators_msa_all <- incubators %>%
  left_join(metro_areas_all, by=c("city"="msa_cities", "state"="msa_states"))

write.csv(incubators_msa_all, "incubators_msa_all.csv", row.names = FALSE, na = "")
```
